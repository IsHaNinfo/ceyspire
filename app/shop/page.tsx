...
// --- Type Definitions (Needed here to maintain data consistency) ---
interface PriceTier {
    weight_grams: number;
    price_lkr: string;
}
...
export default function CategoryPage() {
    ...
    // State now tracks the currently viewed product for the full page view
    const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);

    // --- Handlers ---
    const handleViewDetails = (product: Product) => {
        // Instead of opening a modal, we set the product to display the full page
        setSelectedProduct(product);
    };

    const handleBackToCategory = () => {
        // Clear the selected product to return to the category grid view
    ...
    if (selectedProduct) {
        // Renders the dedicated product page if a product is selected
        return <ProductDetailPage product={selectedProduct} onBack={handleBackToCategory} />;
    }

    // Renders the category page grid otherwise
    return (
                        ...
                        {displayedProducts.map((product) => (
                            ...
                            >
                                ...
                                <div className="absolute bottom-0 left-0 w-full p-4 bg-black/40 backdrop-blur-sm transition-transform duration-300">
                                    ...
                                    <button
                                        // The onClick handler now simulates navigation
